...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'improveWithGemini') {
    improveMarkdownWithGemini(request.markdown, request.apiKey, request.images, request.model)
      .then(sendResponse)
...
async function improveMarkdownWithGemini(markdown, apiKey, images = [], model = 'gemini-2.5-pro') {
  if (!apiKey) {
...
${markdown}`;
  ...
  try {
    ...
    */
    ...
    console.log('Full Gemini response (first 1000 chars):', improvedContent.substring(0, 1000));
    console.log('===== GEMINI RESPONSE END =====');
    
    // Parse the enhanced response with metadata
    const metadataMatch = improvedContent.match(/## Metadata\n([\s\S]*?)\n\n## Content/);
    ...
    if (metadataMatch) {
      ...
      if (techMatch && techMatch[1] && techMatch[1].toLowerCase() !== 'none') {
        technologies = techMatch[1].split(',').map(tech => tech.trim()).filter(tech => tech && tech.toLowerCase() !== 'none');
      }
      console.log('Technologies found:', technologies);
      
      // Extract programming languages - handle both with and without spaces after colon
      const langMatch = metadataText.match(/Programming_Languages:\s*\[([^\]]*)\]/i);
      if (langMatch && langMatch[1] && langMatch[1].toLowerCase() !== 'none') {
        programmingLanguages = langMatch[1].split(',').map(lang => lang.trim()).filter(lang => lang && lang.toLowerCase() !== 'none');
      }
      ...
      if (tagsMatch && tagsMatch[1] && tagsMatch[1].toLowerCase() !== 'none') {
        tags = tagsMatch[1].split(',').map(tag => tag.trim()).filter(tag => tag && tag.toLowerCase() !== 'none');
      }
      ...
      if (conceptsMatch && conceptsMatch[1] && conceptsMatch[1].toLowerCase() !== 'none') {
        keyConcepts = conceptsMatch[1].split(',').map(concept => concept.trim()).filter(concept => concept && concept.toLowerCase() !== 'none');
      }
      ...
      const difficultyMatch = metadataText.match(/Difficulty_Level:\s*(beginner|intermediate|advanced)/i);
      if (difficultyMatch) {
      ...
      console.log('No metadata match found in response - trying fallback parsing');
      ...
      for (const pattern of techPatterns) {
        ...
        if (match && match[1] && match[1].toLowerCase() !== 'none') {
          const items = match[1].includes('[') ? 
            ...
            match[1].split(',');
          technologies = items.map(tech => tech.trim()).filter(tech => tech && tech.toLowerCase() !== 'none');
      ...
      for (const pattern of langPatterns) {
        ...
        if (match && match[1] && match[1].toLowerCase() !== 'none') {
          const items = match[1].includes('[') ? 
            match[1].replace(/[\[\]]/g, '').split(',') : 
            match[1].split(',');
          programmingLanguages = items.map(lang => lang.trim()).filter(lang => lang && lang.toLowerCase() !== 'none');
      ...
      for (const pattern of tagsPatterns) {
        ...
        if (match && match[1] && match[1].toLowerCase() !== 'none') {
          const items = match[1].includes('[') ? 
            match[1].replace(/[\[\]]/g, '').split(',') : 
            match[1].split(',');
          tags = items.map(tag => tag.trim()).filter(tag => tag && tag.toLowerCase() !== 'none');
      ...
      for (const pattern of conceptPatterns) {
        ...
        if (match && match[1] && match[1].toLowerCase() !== 'none') {
          const items = match[1].includes('[') ? 
            match[1].replace(/[\[\]]/g, '').split(',') : 
            match[1].split(',');
          keyConcepts = items.map(concept => concept.trim()).filter(concept => concept && concept.toLowerCase() !== 'none');
    ...
    const improvedMarkdown = contentMatch ? contentMatch[1].trim() : improvedContent;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (response.status === 503) {
        ...
        console.log(`Model overloaded (503). Retrying in ${delay.toFixed(2)} seconds...`);
      ...
      const delay = (baseDelay * Math.pow(2, attempt)) + Math.random();
      console.log(`Encountered an error. Retrying in ${delay.toFixed(2)} seconds...`, error.message);
      await new Promise(resolve => setTimeout(resolve, delay * 1000));
    }
  }
}
...
async function processAndDownloadWithGemini(request) {
  const { pageContent, url, category, markdown, filename, apiKey, model } = request;

  try {
    // Process with Gemini
    const result = await improveMarkdownWithGemini(markdown, apiKey, pageContent.images || [], model);
    
    let finalContent;
    if (result.improvedMarkdown && typeof result.improvedMarkdown === 'object') {
      const enrichedMetadata = createMetadata(pageContent, url, category, result.improvedMarkdown);
      finalContent = enrichedMetadata + result.improvedMarkdown.content;
    } else {
      // Fallback
